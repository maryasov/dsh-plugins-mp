...
async function api$1(base, path, signal) {
...
function fetchCatalog(base, query, signal) {
	...
	if (query.slugs !== void 0 && query.slugs.length > 0) usp.set("slugs", query.slugs.join(","));
	if (query.sort) usp.set("sort", query.sort);
	usp.set("limit", String(Math.min(query.limit ?? 12, 25)));
	usp.set("page", String(query.page ?? 1));
	return api$1(base, `/plugins?${usp.toString()}`, signal);
}
function fetchDetail(base, slug, signal) {
	return api$1(base, `/plugins/${encodeURIComponent(slug)}`, signal);
}
function fetchSimilar(base, slug, signal) {
	return api$1(base, `/plugins/${encodeURIComponent(slug)}/similar`, signal);
}
/**
...
*/
const UPDATE_API_V1_SCHEMA = "dsh-plugins-mp/update-api/v1";
...
function failureOf(status, body) {
	...
	const message = text(body.error) ?? text(body.stderr) ?? text(body.output) ?? `update failed with HTTP ${String(status)}`;
	return {
		code,
...
var UpdateOperationStoreV1 = class {
	bootId;
	now;
	maxOperations;
	sequence = 0;
	...
	activeId = null;
	constructor(bootId, now = Date.now, maxOperations = 50) {
		this.bootId = bootId;
		this.now = now;
	...
	create(packageName, beforeVersion, rollbackId) {
		...
		while (this.operations.size > this.maxOperations) {
			...
			if (oldestId === void 0 || oldestId === this.activeId) break;
	...
	finish(operationId, ok, failureBody, installedVersion, restartRequired) {
		const operation = this.operations.get(operationId);
	...
	beginRollback(operationId) {
		const operation = this.operations.get(operationId);
		if (operation?.rollbackId === null || operation?.rollbackId === void 0) return null;
		operation.outcome.rollback.state = "running";
		operation.outcome.rollback.detail = null;
		return operation.rollbackId;
	}
	finishRollback(operationId, ok, error, installedVersion) {
		const operation = this.operations.get(operationId);
		if (operation === void 0) return null;
		if (installedVersion !== void 0) operation.installedVersion = installedVersion;
...
const TELEMETRY_ROUTE = "/plugins/dsh-plugins-mp/telemetry-payload";
...
const API_V1_CAPABILITIES_ROUTE = "/plugins/dsh-plugins-mp/api/v1/capabilities";
const API_V1_UPDATES_ROUTE = "/plugins/dsh-plugins-mp/api/v1/updates";
const API_V1_OPERATIONS_ROUTE = "/plugins/dsh-plugins-mp/api/v1/operations";
const API_V1_ROLLBACK_ROUTE = "/plugins/dsh-plugins-mp/api/v1/rollback";
const API_V1_RESTART_ROUTE = "/plugins/dsh-plugins-mp/api/v1/restart";
...
*/
function sendTelemetryEvent(apiBase, state, kind, slug, version) {
	if (state?.telemetry === false || state?.fingerprint === void 0) return;
	fetch(`${apiBase}/telemetry/event`, {
		method: "POST",
		headers: { "content-type": "application/json" },
		body: JSON.stringify({
...
function mountRoutes(ctx, config = {}, runtime) {
	...
	ctx.inject(["webServer"], (sctx) => {
		sctx.effect(() => {
			...
			const stopInstall = webServer.register({
				...
				handler: async (req, res) => {
					...
					try {
						...
						if (outcome.ok) {
							const installedItem = installedName !== null ? readInstalled(resolveProfileDir({ profile })).find((item) => item.name === installedName) : void 0;
			...
			const stopTelemetry = webServer.register({
				...
				handler: (_req, res) => {
					...
					json(res, 200, {
						...
						dshVersion: dshHostInfo()?.version ?? null,
						plugins: readInstalled(resolveProfileDir(config)).filter((item) => item.source === "npm" || item.source === "github").map((item) => ({
							slug: item.name,
			...
			const installedVersionOf = (profileDir, name) => readInstalled(profileDir).find((item) => item.name === name)?.version ?? null;
			...
			const stopV1Updates = webServer.register({
				...
				handler: async (req, res) => {
					...
					if (req.method === "GET") {
						...
						const item = readInstalled(resolveProfileDir(config)).find((entry) => entry.name === name);
						if (item === void 0 || item.source !== "npm" || item.version === null) {
							json(res, 404, {
								schema: UPDATE_API_V1_SCHEMA,
								error: "plugin is not installed (or not an npm package)"
							});
							return;
						}
					...
					runDshPlugin(profileName(config), ["add", `${name}@latest`]).then((outcome) => {
						const after = installedVersionOf(profileDir, name);
						operationsV1.finish(operation.operationId, outcome.ok, outcome.ok ? null : {
							error: outcome.output,
							timedOut: outcome.timedOut
						}, after, outcome.ok && after !== before);
			...
			const stopV1Operations = webServer.register({
				...
				handler: (req, res) => {
					...
					if (operation === null) {
						json(res, 404, {
							schema: UPDATE_API_V1_SCHEMA,
							error: "operation not found in this host process"
						});
						return;
					}
					json(res, 200, {
			...
			const stopV1Rollback = webServer.register({
				...
				handler: async (req, res) => {
					...
					if (snapshotId === null) {
						json(res, 409, {
							schema: UPDATE_API_V1_SCHEMA,
							error: "rollback is not available for this operation"
						});
						return;
					}
					const profileDir = resolveProfileDir(config);
					const result = snapshotRestore(profileDir, snapshotId);
					const operation = operationsV1.finishRollback(operationId, result.ok, result.error ?? null, result.ok ? installedVersionOf(profileDir, operationsV1.get(operationId)?.packageName ?? "") : void 0);
...
function registerTools(ctx, apiBase) {
	...
	register(defineTool({
		...
		parameters: {
			...
			section: {
				type: "string",
				description: "Catalog section: \"plugin\" (default), \"skill\" or \"app\" — sections have disjoint category sets.",
				enum: [
					"plugin",
					"skill",
					"app"
			...
			category: {
				type: "string",
				description: "Category slug filter (e.g. \"ui\", \"tools\", \"memory\" for plugins; \"agents\", \"design\" for skills; \"desktop\", \"utilities\" for apps)."
			},
			profile: {
				type: "string",
	...
	register(defineTool({
		...
		parameters: {
			by: {
				type: "string",
				description: "Ranking metric. installs = real anonymous install counts (trailing 7 days).",
				enum: [
					"installs",
					"stars",
					"downloads",
		...
		async execute(args, exec) {
			...
			const by = [
				...
				"downloads",
				"updated"
			].includes(args.by) ? args.by : "installs";
			const limit = Math.min(args.limit ?? 10, 25);
			let metric = by;
			let cards = [];
			if (by === "installs") try {
				const trend = await api(apiBase, "/trending?days=7&limit=" + String(limit), exec.signal);
				if (trend.length > 0) {
					const catalog = await fetchCatalog(apiBase, {
						slugs: trend.map((entry) => entry.slug),
						limit: Math.max(limit, trend.length)
					}, exec.signal);
					const bySlug = new Map(catalog.items.map((card) => [card.slug, card]));
					cards = trend.map((entry) => bySlug.get(entry.slug)).filter((card) => card !== void 0);
					metric = "installs";
				} else metric = "stars";
			} catch {
				metric = "stars";
			...
			if (metric !== "installs") {
				cards = (await fetchCatalog(apiBase, {
					sort: metric === "downloads" ? "stars" : metric,
					limit
				}, exec.signal)).items;
				if (metric === "downloads") cards = [...cards].sort((a, b) => b.npmDownloadsWeek - a.npmDownloadsWeek);
			}
			return {
